...
public class APIIRSymTable {
     ...
     * @return *
     */
    public ElementTable findElementIRGen(TableSymbol table, String str, int falseRow) {
        if (str == null || str.length() == 0) {
            return null;
        }
        while (table != null) {
            if (table.contain(str)) {
                ElementTable elementTable = table.getElement(str);
                if (elementTable.getFalseRow() <= falseRow) {
                    return elementTable;
                }
            }
            table = table.getFather();
     ...
     * 判断变量是否是0维常量
     */
    public boolean isConstNum(TableSymbol tableSymbol, String name, int falseRow) {
        ElementTable elementTable = findElementIRGen(tableSymbol, name, falseRow);
     ...
     * 在isConstNum的前提下得到值
     */
    public int getConstNum(TableSymbol tableSymbol, String name, int falseRow) {
        ElementTable elementTable = findElementIRGen(tableSymbol, name, falseRow);
     ...
     *找到二维数组第2位的长度
     */
    public int findTwoDimArrayLen(TableSymbol tableSymbol, String name, int falseRow) {
        ElementTable elementTable = findElementIRGen(tableSymbol, name, falseRow);
